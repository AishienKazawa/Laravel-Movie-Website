    ...
    <div class="w-full grid" style="grid-template-rows: auto 1fr">
        ...
        <div class="w-screen h-full flex justify-center items-center py-10 px-10 xl:px-36">
            <div class="grid sm:w-9/12 md:w-9/12 lg:w-6/12 2xl:w-5/12">
                ...
                <h1 class="text-3xl md:text-4xl 2xl:text-4xl font-bold my-2">
                    Create a password and start your membership.
                </h1>

                <p>Unlock unlimited movies and exclusive content by joining now!</p>

                <form method="POST" action="{{ route('signup.create-password') }}" class="mt-5 grid gap-5 md:gap-4">
                    @csrf
                    <div>
                        <div>Email: <span class="font-semibold">{{ session('email') }}</span></div>
                        @if (session('error'))
                            <p class="text-xs mt-2 text-red-500">{{ session('error') }}</p>
                        @endif
                    </div>
                    <div>
